...
struct PortfolioUnitInvestmentView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            // Enhanced Follow-on Investments Section with Batch Management
            VStack(spacing: 12) {
                HStack {
            ...
            let currentErrors = validateInputs()
            if !currentErrors.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        ...
                        Button(showingValidationDetails ? "Hide Details" : "Show Details") {
                            showingValidationDetails.toggle()
                        }
                        .font(.caption)
                        .foregroundColor(.blue)
                    }
                    ...
                    if showingValidationDetails {
                        VStack(alignment: .leading, spacing: 4) {
                            ForEach(currentErrors, id: \.self) { error in
                        ...
                        Text("\(currentErrors.count) validation issue\(currentErrors.count == 1 ? "" : "s") found")
                            .font(.caption)
                            .foregroundColor(.orange)
                    }
                }
