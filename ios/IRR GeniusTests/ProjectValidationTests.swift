...
final class ProjectValidationTests: XCTestCase {
    ...
    func testLongNameValidation() {
        ...
        XCTAssertThrowsError(try Project(name: longName)) { error in
            // Then should throw validation error
            XCTAssertTrue(error is ProjectValidationError)
            if case let ProjectValidationError.invalidName(reason) = error {
    ...
    func testLongDescriptionValidation() {
        ...
        XCTAssertThrowsError(try Project(
            ...
            // Then should throw validation error
            XCTAssertTrue(error is ProjectValidationError)
            if case let ProjectValidationError.invalidDescription(reason) = error {
                XCTAssertTrue(reason.contains("too long"))
            } else {
    ...
    func testInvalidColorValidation() {
        ...
        for invalidColor in invalidColors {
            ...
            XCTAssertThrowsError(try Project(
                ...
                // Then should throw validation error
                XCTAssertTrue(error is ProjectValidationError)
                if case let ProjectValidationError.invalidColor(reason) = error {
    ...
    func testProjectStatistics() throws {
        ...
        let calculations = try [
            createTestCalculation(projectId: project.id, isComplete: true, type: .calculateIRR),
            createTestCalculation(projectId: project.id, isComplete: false, type: .calculateOutcome),
            createTestCalculation(projectId: project.id, isComplete: true, type: .calculateIRR),
            createTestCalculation(projectId: UUID(), isComplete: true, type: .calculateInitial), // Different project
        ]
